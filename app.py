...
def fetch_tmdb_data(total_target=12000):
    """
    Fetch 10k+ unique titles as fast as possible using aggressive parallelization
    and connection pooling across multiple high-yield TMDB endpoints.
    """
    all_content = []

    # Diverse endpoints reduce duplicate overlap and hit the 10k target faster
    endpoints = [
        ("movie/popular", {}),
        ("movie/top_rated", {}),
        ("movie/now_playing", {}),
    ...
    pages_per_endpoint = int(
        np.ceil(total_target / (results_per_page * len(endpoints)))
    )
    pages_per_endpoint = min(max(pages_per_endpoint, 30), 500)  # TMDB caps at 500 pages
    ...
    def fetch_page(endpoint, page, params):
    ...
    # Build all tasks (endpoint + page combos)
    tasks = []
    for endpoint, params in endpoints:
        for page in range(1, pages_per_endpoint + 1):
            tasks.append((endpoint, page, params))

    # Parallel fetch; high worker count keeps total wall time low
    with ThreadPoolExecutor(max_workers=64) as executor:
        futures = {
            executor.submit(fetch_page, endpoint, page, params): (endpoint, page)
            for endpoint, page, params in tasks
        }
    ...
    # De-duplicate and post-process
    df = pd.DataFrame(all_content).drop_duplicates(subset=["id"])

    # In practice this will usually be 5000+ unique titles with the settings above
    # but if you want to be absolutely sure, you can optionally log len(df)
    ...
    df["genre_names"] = df["genre_ids"].apply(
        lambda x: [GENRE_MAP.get(g, "Unknown") for g in x]
        if isinstance(x, list)
        else []
    )
    df["primary_genre"] = df["genre_names"].apply(
    ...
    df["engagement_score"] = (
        df["popularity"] * 0.4 + (df["vote_average"] * df["vote_count_factor"]) * 0.6
    )
    ...
    df["poster_url"] = df["poster_path"].apply(
        lambda x: f"{TMDB_IMAGE_BASE}{x}" if pd.notna(x) else None
    )
    df["backdrop_url"] = df["backdrop_path"].apply(
        lambda x: f"{TMDB_BACKDROP_BASE}{x}" if pd.notna(x) else None
    )
...
def create_insight_panel(title, insights):
    ...
    st.markdown(
        f'<div class="insight-panel"><div class="insight-title">{title}</div>{insights_html}</div>',
        unsafe_allow_html=True,
    )
            ...
            if idx < len(posters):
                with columns[col_idx]:
                    ...
                    if p["poster_url"]:
                        ...
                        st.markdown(
                            f'<div style="text-align: center; color: #FFFFFF; font-size: 0.9rem; margin-top: 8px;">{p["title"][:30]}</div>',
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            f'<div style="text-align: center; color: #B3B3B3; font-size: 0.8rem;">⭐ {p["vote_average"]:.1f} | 🔥 {p["popularity"]:.0f}</div>',
                            unsafe_allow_html=True,
                        )
...
def apply_filters(df, filters, ignore_content_type=False):
    ...
    if not ignore_content_type and filters["content_type"] != "All":
        df_filtered = df_filtered[
            df_filtered["content_type"] == filters["content_type"]
        ]
...
def dashboard_1_content_universe(df, filters):
    ...
    df_filtered = apply_filters(
        df, filters, ignore_content_type=True
    )  # Show all content types

    st.title("🌍 Content Universe Overview")
    st.markdown(
        '<p class="subtitle">Explore the market landscape - what content exists and where</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="dashboard-description"><strong>Business Problem:</strong> Understand current market landscape. This dashboard provides a 30,000-foot view of content distribution, quality-popularity dynamics, and market composition. Use this to identify high-level patterns before diving deeper into specific opportunities.</div>',
        unsafe_allow_html=True,
    )
    ...
    shawshank = df_filtered[
        df_filtered["title"].str.contains("Shawshank", case=False, na=False)
    ]
    ...
    movie_quote = movie_quotes.get(
        featured_title["title"],
        featured_title.get(
            "tagline", "Discover the most acclaimed content in our catalog"
        ),
    )
    ...
    with tab1:
        ...
        if len(movies) > 0:
            ...
            with col1:
                ...
                st.markdown(
                    "**Chart Analysis:** This horizontal bar chart shows the distribution of movie titles across the top 10 genres. Each bar is color-coded according to the genre for easy visual identification. Longer bars indicate more content in that genre, which could signal either opportunity (popular category) or saturation (overcrowded market)."
                )
                ...
                fig = go.Figure(
                    go.Bar(
                        ...
                        orientation="h",
                        marker=dict(
                            color=colors_list, line=dict(color="#FFFFFF", width=0.5)
                        ),
                ...
                fig.update_layout(
                    xaxis=dict(
                        title="Number of Titles", color="#B3B3B3", gridcolor="#262626"
                    ),
            ...
            with col2:
                ...
                fig = px.scatter(
                    ...
                    color_discrete_sequence=[
                        get_genre_color(g) for g in genre_stats["Genre"]
                    ],
                ...
                fig.update_layout(
                    xaxis=dict(
                        title="Avg Popularity (Reach)",
                        color="#B3B3B3",
                        gridcolor="#262626",
                    ),
                    yaxis=dict(
                        title="Avg Rating (Quality)",
                        color="#B3B3B3",
                        gridcolor="#262626",
                    ),
                    paper_bgcolor="#0A0A0A",
                ...
                pop_median = genre_stats["Avg_Pop"].median()
                rating_median = genre_stats["Avg_Rating"].median()
                sweet_spot = genre_stats[
                    (genre_stats["Avg_Pop"] >= pop_median)
                    & (genre_stats["Avg_Rating"] >= rating_median)
                ]
                if len(sweet_spot) > 0:
                    best_genre = sweet_spot.nlargest(1, "Avg_Pop").iloc[0]["Genre"]
                else:
                    best_genre = genre_stats.nlargest(1, "Avg_Pop").iloc[0]["Genre"]
                niche_high_quality = genre_stats[
                    (genre_stats["Avg_Pop"] < pop_median)
                    & (genre_stats["Avg_Rating"] >= rating_median)
                ]

                create_insight_panel(
                    "Quality-Popularity Dynamics",
                    [
                        f"Sweet spot now led by <span class='insight-highlight'>{best_genre}</span> (above-median popularity and quality).",
                        f"{len(niche_high_quality)} niche genres score high on quality but below-median reach — candidates for targeted growth.",
                        f"Use large bubbles in the sweet spot for safe bets; nurture high-quality niche bubbles with differentiated marketing. Data reflects {filters['year_range'][0]}–{filters['year_range'][1]} selections.",
                    ],
                )

            st.subheader("Top Performing Movies")
            st.markdown(
                "**Grid Analysis:** These are the highest-performing movies by popularity score. Study these titles to understand current market preferences, successful themes, and audience engagement patterns. Hover for detailed metrics."
            )
            display_poster_grid(movies, limit=12, cols=4)
        else:
            st.info(
                "No movies match current filters. Try adjusting your filter criteria."
            )
    ...
    with tab2:
        ...
        if len(tv) > 0:
            ...
            with col1:
                ...
                st.markdown(
                    "**Chart Analysis:** Distribution of TV show titles across genres. Compare this to movie distribution to identify platform-specific opportunities. TV audiences may have different genre preferences than movie audiences."
                )
                ...
                fig = go.Figure(
                    go.Bar(
                        ...
                        orientation="h",
                        marker=dict(
                            color=[get_genre_color(g) for g in genre_dist_tv.index],
                ...
                fig.update_layout(
                    xaxis=dict(
                        title="Number of Shows", color="#B3B3B3", gridcolor="#262626"
                    ),
                    yaxis=dict(title="", color="#B3B3B3"),
                    paper_bgcolor="#0A0A0A",
                    plot_bgcolor="#0A0A0A",
                ...
                tv_top_genre = genre_dist_tv.index[0]
                tv_top_count = genre_dist_tv.values[0]
                tv_top3_share = genre_dist_tv.values[:3].sum() / len(tv) * 100

                create_insight_panel(
                    "TV Market Composition Insights",
                    [
                        f"<span class='insight-highlight'>{tv_top_genre}</span> leads with {tv_top_count} titles.",
                        f"Top 3 genres make up <span class='insight-highlight'>{tv_top3_share:.1f}%</span> of TV supply; long tail shows room for differentiation.",
                        "Use leader genres for scale plays; test under-represented genres for niche growth.",
                    ],
                )
            ...
            with col2:
                ...
                st.markdown(
                    "**Chart Analysis:** TV shows mapped by popularity vs quality. Look for differences from movie patterns. Some genres perform better in TV format (e.g., Drama for character development, Comedy for episodic formats)."
                )

                genre_stats_tv = (
                    tv.groupby("primary_genre")
                    .agg({"popularity": "mean", "vote_average": "mean", "id": "count"})
                    .reset_index()
                    .nlargest(10, "popularity")
                )
                ...
                fig = px.scatter(
                    ...
                    color="Genre",
                    color_discrete_sequence=[
                        get_genre_color(g) for g in genre_stats_tv["Genre"]
                    ],
                )

                fig.update_layout(
                    xaxis=dict(
                        title="Avg Popularity", color="#B3B3B3", gridcolor="#262626"
                    ),
                    yaxis=dict(
                        title="Avg Rating", color="#B3B3B3", gridcolor="#262626"
                    ),
                    paper_bgcolor="#0A0A0A",
                    plot_bgcolor="#0A0A0A",
                    font=dict(color="#FFFFFF"),
                ...
                tv_pop_median = genre_stats_tv["Avg_Pop"].median()
                tv_rating_median = genre_stats_tv["Avg_Rating"].median()
                tv_sweet = genre_stats_tv[
                    (genre_stats_tv["Avg_Pop"] >= tv_pop_median)
                    & (genre_stats_tv["Avg_Rating"] >= tv_rating_median)
                ]
                tv_leader = (
                    tv_sweet.nlargest(1, "Avg_Pop").iloc[0]["Genre"]
                    if len(tv_sweet) > 0
                    else genre_stats_tv.nlargest(1, "Avg_Pop").iloc[0]["Genre"]
                )
                tv_niche = genre_stats_tv[
                    (genre_stats_tv["Avg_Pop"] < tv_pop_median)
                    & (genre_stats_tv["Avg_Rating"] >= tv_rating_median)
                ]
                tv_year_min = (
                    int(tv["year"].min())
                    if tv["year"].notna().any()
                    else filters["year_range"][0]
                )
                tv_year_max = (
                    int(tv["year"].max())
                    if tv["year"].notna().any()
                    else filters["year_range"][1]
                )

                create_insight_panel(
                    "TV Quality-Popularity Dynamics",
                    [
                        f"Sweet spot led by <span class='insight-highlight'>{tv_leader}</span> (above-median popularity and quality).",
                        f"{len(tv_niche)} high-quality niches have below-median reach — target them with focused campaigns.",
                        f"Insights reflect TV titles in years {tv_year_min}–{tv_year_max}; adjust the slider to re-benchmark TV performance.",
                    ],
                )

            st.subheader("Top Performing TV Shows")
            st.markdown(
                "**Grid Analysis:** Highest-engagement TV shows. Compare casting, themes, and formats against movie top performers to understand format-specific success factors."
            )
            display_poster_grid(tv, limit=12, cols=4)
        else:
            st.info(
                "No TV shows match current filters. Try adjusting your filter criteria."
            )
...
def dashboard_2_performance_deepdive(df, filters):
    ...
    st.markdown(
        '<p class="subtitle">Understand what drives content success and identify key patterns</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="dashboard-description"><strong>Business Problem:</strong> Analyze WHY some content outperforms others by examining correlations, seasonal patterns, and statistical anomalies. Use these insights to replicate success factors in future productions and avoid common pitfalls.</div>',
        unsafe_allow_html=True,
    )
    ...
    col4.metric(
        "Elite Titles (Top 25%)",
        f"{len(df_filtered[df_filtered['engagement_score'] > df_filtered['engagement_score'].quantile(0.75)]):,}",
    )
    ...
    with col1:
        ...
        st.markdown(
            "**Chart Analysis:** Statistical outlier detection using Isolation Forest, a multi-dimensional anomaly detection model. Red dots represent titles with unusual patterns across popularity, rating, vote count, and engagement score. Log scale on X-axis helps visualize extreme popularity differences. These anomalies may represent surprise hits, cult favorites, or titles with unusual audience dynamics."
        )

        df_copy = df_filtered.copy()

        # Multi-dimensional anomaly detection using IsolationForest
        feature_cols = ["popularity", "vote_average", "vote_count", "engagement_score"]
        iso_features = df_copy[feature_cols].fillna(0)
        ...
        anomaly_labels = iso.fit_predict(iso_features)
        ...
        fig.update_layout(
            xaxis=dict(
                title="Popularity Score (log scale)",
                color="#B3B3B3",
                gridcolor="#262626",
                type="log",
            ),
    ...
    with col2:
        ...
        st.markdown(
            "**Chart Analysis:** Heatmap showing statistical correlations between key performance metrics. Values range from 0 (no correlation) to 1 (perfect correlation). Higher values (red) = stronger relationships. This reveals which factors actually drive popularity. Use this to prioritize your investment areas."
        )

        # Use Spearman to capture monotonic relationships and keep symmetric color mapping
        corr_data = df_filtered[
            ["popularity", "vote_average", "vote_count", "engagement_score"]
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                autorange="reversed"
            ),  # keep diagonal visually top-left to bottom-right
        )
        ...
        # Avoid division noise when rating correlation is near zero
        ratio = (
            pop_vote_corr / pop_rating_corr
            if abs(pop_rating_corr) > 1e-3
            else float("inf")
        )
        ratio_text = (
            f"{ratio:.1f}x MORE" if np.isfinite(ratio) else "substantially MORE"
        )

        create_insight_panel(
            "Correlation Intelligence",
            [
                f"<span class='insight-highlight'>Vote Count → Popularity: {pop_vote_corr:.2f}</span> ({describe_corr(pop_vote_corr)} correlation)",
                f"<span class='insight-highlight'>Rating → Popularity: {pop_rating_corr:.2f}</span> ({describe_corr(pop_rating_corr)} correlation)",
                f"Volume of engagement matters <span class='insight-highlight'>{ratio_text}</span> than quality scores for driving popularity.",
                "Actionable insight: Marketing & distribution (which drive vote counts) tend to outweigh content quality alone. Invest in both, but don't neglect go-to-market strategy.",
            ],
        )
    ...
    with col1:
        ...
        monthly_perf = (
            df_filtered.groupby("month_name")["engagement_score"].mean().reset_index()
        )
        monthly_perf["month_name"] = pd.Categorical(
            monthly_perf["month_name"], categories=month_order, ordered=True
        )
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                title="Avg Engagement Score", color="#B3B3B3", gridcolor="#262626"
            ),
            paper_bgcolor="#0A0A0A",
            plot_bgcolor="#0A0A0A",
            font=dict(color="#FFFFFF"),
        ...
        best_month = monthly_perf.loc[
            monthly_perf["engagement_score"].idxmax(), "month_name"
        ]
        worst_month = monthly_perf.loc[
            monthly_perf["engagement_score"].idxmin(), "month_name"
        ]
    ...
    with col2:
        ...
        votes_pop_corr = corr_data.loc["vote_count", "popularity"]
        log_votes = np.log1p(df_filtered["vote_count"])
        log_pop = np.log1p(df_filtered["popularity"])
        log_corr = np.corrcoef(log_votes, log_pop)[0, 1]

        def describe_strength(val):
            abs_val = abs(val)
            if abs_val >= 0.6:
                return "strong"
            if abs_val >= 0.3:
        ...
        # Identify "top-right" high performers on log axes
        hi_vote = df_filtered["vote_count"] >= df_filtered["vote_count"].quantile(0.85)
        hi_pop = df_filtered["popularity"] >= df_filtered["popularity"].quantile(0.85)
        hi_rating = df_filtered["vote_average"] >= 8
        holy_grail = hi_vote & hi_pop & hi_rating
        holy_pct = holy_grail.mean() * 100

        growth_shape = (
            "exponential (log-log correlation {:.2f})".format(log_corr)
            if log_corr >= 0.6
            else "sublinear (log-log correlation {:.2f})".format(log_corr)
        )

        create_insight_panel(
            "Engagement Correlation Findings",
            [
                f"Votes → popularity = {votes_pop_corr:.2f} ({describe_strength(votes_pop_corr)}).",
                f"<span class='insight-highlight'>Premium hits:</span> {holy_pct:.1f}% sit in top 15% votes & popularity with rating ≥ 8.0.",
                f"Growth pattern: {growth_shape}; pushing into the upper cluster needs multiplicative engagement.",
            ],
        )
...
def dashboard_3_market_opportunities(df, filters):
    ...
    st.markdown(
        '<p class="subtitle">Identify market gaps, saturation points, and strategic opportunities</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="dashboard-description"><strong>Business Problem:</strong> Where should we focus resources? This dashboard reveals underserved markets (GREEN zones), oversaturated genres (RED zones), winning genre combinations, and optimal release timing. Use this to identify high-ROI opportunities before competitors discover them.</div>',
        unsafe_allow_html=True,
    )
    ...
    pareto_threshold = (
        df_filtered.groupby("primary_genre")["popularity"].sum().nlargest(5).sum()
        / df_filtered["popularity"].sum()
    )
    ...
    st.markdown(
        "**Chart Analysis:** Bubble chart mapping market saturation. X-axis = volume of content, Y-axis = average performance, Bubble size = opportunity score. GREEN zone (top-left) = Low competition + high performance = OPPORTUNITY. RED zone (bottom-right) = High competition + low performance = AVOID. Use this to prioritize genre investments."
    )

    saturation_data = (
        df_filtered.groupby("primary_genre")
        .agg({"id": "count", "popularity": "mean", "engagement_score": "mean"})
        .reset_index()
    )
    saturation_data.columns = ["Genre", "Volume", "Avg_Pop", "Avg_Eng"]
    saturation_data["Opportunity"] = (
        saturation_data["Avg_Pop"] / (saturation_data["Volume"] ** 0.5)
    ) * 10

    saturation_data["Zone"] = "Moderate"
    saturation_data.loc[
        (saturation_data["Volume"] > saturation_data["Volume"].quantile(0.66))
        & (saturation_data["Avg_Pop"] < saturation_data["Avg_Pop"].median()),
        "Zone",
    ] = "Oversaturated"
    saturation_data.loc[
        (saturation_data["Volume"] < saturation_data["Volume"].quantile(0.33))
        & (saturation_data["Avg_Pop"] > saturation_data["Avg_Pop"].median()),
        "Zone",
    ] = "Opportunity"
    ...
    fig.update_layout(
        xaxis=dict(
            title="Market Volume (# Titles)", color="#B3B3B3", gridcolor="#262626"
        ),
    ...
    opp_genres = saturation_data[saturation_data["Zone"] == "Opportunity"][
        "Genre"
    ].tolist()
    over_genres = saturation_data[saturation_data["Zone"] == "Oversaturated"][
        "Genre"
    ].tolist()
    ...
    with col1:
        ...
        st.markdown(
            "**Chart Analysis:** Demonstrates the Pareto principle: typically 20% of genres generate 80% of engagement. Red bars = genres contributing to the first 80% (vital few). Use this to focus resources on proven winners while maintaining strategic diversity in the long tail."
        )

        genre_totals = (
            df_filtered.groupby("primary_genre")["engagement_score"]
            .sum()
            .sort_values(ascending=False)
            .reset_index()
        )
        genre_totals["Cumulative_%"] = (
            genre_totals["engagement_score"].cumsum()
            / genre_totals["engagement_score"].sum()
            * 100
        )
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker=dict(
                    color=[
                        COLORS["danger"] if cp < 80 else COLORS["primary"]
                        for cp in genre_totals["Cumulative_%"][:10]
                    ]
                ),
        ...
        fig.update_yaxes(
            title_text="Total Engagement",
            color="#B3B3B3",
            gridcolor="#262626",
            secondary_y=False,
        )
        fig.update_yaxes(
            title_text="Cumulative %", color="#B3B3B3", range=[0, 100], secondary_y=True
        )
    ...
    with col2:
        ...
        st.markdown(
            "**Chart Analysis:** Heatmap showing average engagement by month × genre combination. Red cells = optimal release windows. Blue cells = weak periods. Use this to time your genre-specific releases for maximum impact. Each cell represents historical performance data."
        )

        # Filter out null dates and prepare data
        df_with_dates = df_filtered[
            df_filtered["month_name"].notna() & (df_filtered["month_name"] != "Unknown")
        ]
        month_genre = (
            df_with_dates.groupby(["month_name", "primary_genre"])["engagement_score"]
            .mean()
            .reset_index()
        )
        month_order = [
        ...
        pivot = month_genre.pivot(
            index="month_name", columns="primary_genre", values="engagement_score"
        )
...
def dashboard_4_recommendation_copilot(df, filters):
    ...
    with col1:
        ...
        selected_genre = st.selectbox(
            ...
            all_genres,
            index=0
            if "Science Fiction" not in all_genres
            else all_genres.index("Science Fiction"),
        )
    ...
    with col3:
        budget_proxy = st.slider(
            "Budget Level", 1, 10, 7, help="1=Low Budget, 10=Blockbuster"
        )
    ...
    # ------------- Popularity Regression: LightGBM / XGBoost -------------
    if len(genre_data) >= 30:
        # Feature set - we keep it simple & robust
        X_reg = genre_data[["vote_average", "vote_count"]].fillna(
            genre_data[["vote_average", "vote_count"]].mean()
        )
        y_reg = genre_data["popularity"]

        try:
            # Primary: LightGBM (fast, strong)
    ...
    if len(genre_data) >= 50:
        clf_data = genre_data.copy()
        clf_data = clf_data[clf_data["engagement_score"].notna()].copy()

        if "quarter" not in clf_data.columns:
            # quarter already exists globally, but ensure just in case
            clf_data["quarter"] = clf_data["month"].apply(
                lambda m: (m - 1) // 3 + 1 if pd.notna(m) else np.nan
            )

        X_clf = clf_data[["vote_average", "vote_count", "quarter"]].fillna(
            clf_data[["vote_average", "vote_count", "quarter"]].median()
        )
        y_clf = (clf_data["engagement_score"] > threshold).astype(int)
        ...
        # Feature vector for the proposed title
        avg_rating = genre_data["vote_average"].mean()
        avg_votes = genre_data["vote_count"].mean()
        x_new = np.array([[avg_rating, avg_votes, q_num]])

        hit_prob = float(rf_clf.predict_proba(x_new)[0, 1]) * 100.0
    else:
        # Fallback: empirical hit rate or default
        hit_prob = (
            len(genre_data[genre_data["engagement_score"] > threshold])
            / len(genre_data)
            * 100
            if len(genre_data) > 0
            else 50.0
    ...
    col3.markdown(
        f'<div style="text-align:center;margin-top:20px;"><span class="{risk_class}">{risk_level} Risk</span></div>',
        unsafe_allow_html=True,
    )
    col4.metric(
        "Genre Avg Rating",
        f"{genre_data['vote_average'].mean():.1f}/10" if len(genre_data) > 0 else "N/A",
    )
    ...
    with col1:
        ...
        st.markdown(
            "**Chart Analysis:** ML model prediction showing how budget levels affect expected popularity. Green curve = predicted trajectory. Red star = your selected budget level. Shows diminishing returns at higher budgets. Use to optimize investment levels."
        )
        ...
        fig.update_layout(
            xaxis=dict(
                title="Budget Level (1-10)", color="#B3B3B3", gridcolor="#262626"
            ),
            yaxis=dict(
                title="Predicted Popularity", color="#B3B3B3", gridcolor="#262626"
            ),
            paper_bgcolor="#0A0A0A",
            plot_bgcolor="#0A0A0A",
            font=dict(color="#FFFFFF"),
        ...
        best_budget_idx = int(np.argmax(popularity_forecast))
        best_budget_level = budget_range[best_budget_idx]
        best_pop = popularity_forecast[best_budget_idx]
        growth_mid = popularity_forecast[5] - popularity_forecast[4]
        growth_end = popularity_forecast[-1] - popularity_forecast[-2]

        create_insight_panel(
            "Investment ROI Curve",
            [
                f"Your budget {budget_proxy}/10 predicts <span class='insight-highlight'>{predicted_popularity:.0f}</span> popularity.",
                f"Peak on the curve occurs around <span class='insight-highlight'>{best_budget_level}/10</span> (~{best_pop:.0f} predicted popularity).",
                f"Returns slow down at the top: gain from 5→6 is {growth_mid:.0f} vs {growth_end:.0f} from 9→10. Pick a level where marginal gain justifies spend.",
            ],
        )
    ...
    with col2:
        ...
        st.markdown(
            "**Chart Analysis:** Radar shows the relative balance across six metrics (0–100 scaled). Look for symmetry vs spikes/dips; the shape highlights trade-offs between reach, quality, engagement, scale, votes, and recency."
        )

        if len(genre_data) > 0:
            metrics = {
                "Popularity": genre_data["popularity"].mean()
                / df_filtered["popularity"].max()
                * 100,
                "Quality": genre_data["vote_average"].mean() / 10 * 100,
                "Engagement": genre_data["engagement_score"].mean()
                / df_filtered["engagement_score"].max()
                * 100,
                "Market Volume": len(genre_data) / len(df_filtered) * 100 * 5,
                "Vote Count": genre_data["vote_count"].mean()
                / df_filtered["vote_count"].max()
                * 100,
                "Recency": (2025 - genre_data["year"].mean()) / 25 * 100
                if genre_data["year"].notna().any()
                else 50,
            }
        else:
            metrics = {
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True, range=[0, 100], color="#B3B3B3", gridcolor="#262626"
                ),
        ...
        top_metric = categories[int(np.argmax(values))]
        low_metric = categories[int(np.argmin(values))]

        create_insight_panel(
            "Multi-Dimensional Profile Analysis",
            [
                f"Strongest dimension: <span class='insight-highlight'>{top_metric}</span>. Leverage it as a core positioning pillar.",
                f"Weakest dimension: <span class='insight-highlight'>{low_metric}</span>. Mitigate with creative, casting, or marketing choices.",
                "Balanced shapes = versatile genres; spiky shapes need packaging to offset weak sides.",
            ],
        )
    ...
    st.markdown(
        "**Analysis:** Top 12 similar titles (same genre + type) by popularity. Study these to understand what works in your target category. Look for common themes, talent, marketing approaches."
    )
    ...
    if len(similar) > 0:
        ...
        st.info(
            f"No {content_type} titles found in {selected_genre}. Try different parameters."
        )
...
def dashboard_5_predictive_forecasting(df, filters):
    ...
    st.markdown(
        '<p class="subtitle">ML-powered predictions for future trends and emerging patterns</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="dashboard-description"><strong>Business Problem:</strong> What will the market look like in 6-12 months? This dashboard uses Prophet Based Regression ML to forecast genre trends, lifecycle models to predict content decay, and momentum indicators to identify rising/falling categories. Use for strategic planning and pipeline development.</div>',
        unsafe_allow_html=True,
    )

    st.subheader("12-Month Genre Engagement Forecast (Prophet Based Regression)")
    st.markdown(
        "**Chart Analysis:** Machine learning forecasts showing predicted engagement trends for top 6 genres over next 12 months. Each line represents one genre. Upward slopes = growth opportunities. Flat/declining = reconsider or wait. Models use Prophet Based Regression for stability."
    )

    # Build forecasts
    # Build forecasts with Prophet
    forecast_data = df_filtered[df_filtered["year"].notna()].copy()

    # Construct a proper monthly date index
    forecast_data["date"] = pd.to_datetime(
        dict(year=forecast_data["year"], month=forecast_data["month"], day=1),
        errors="coerce",
    )
    forecast_data = forecast_data[forecast_data["date"].notna()]

    forecast_data = (
        forecast_data.groupby(["primary_genre", "date"])["engagement_score"]
        .mean()
        .reset_index()
    )
    ...
    for genre in top_genres:
        ...
        # Need at least ~12 points for a meaningful monthly Prophet model
        if len(genre_ts) >= 12:
            ts = genre_ts.rename(columns={"date": "ds", "engagement_score": "y"})[
                ["ds", "y"]
            ]

            # Prophet handles non-linear trend + yearly seasonality
            m = Prophet(
            ...
            future_pred = forecast.tail(12)["yhat"].values
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="Predicted Engagement Score", color="#B3B3B3", gridcolor="#262626"
        ),
        paper_bgcolor="#0A0A0A",
        plot_bgcolor="#0A0A0A",
        font=dict(color="#FFFFFF"),
    ...
    if predictions:
        growth_rates = {
            g: (p[-1] - p[0]) / p[0] * 100 for g, p in predictions.items() if p[0] > 0
        }
        if growth_rates:
    ...
    st.markdown(
        "**Chart Analysis:** Velocity ranking = Popularity per day since release. Red bars = viral content gaining traction rapidly. Study these to understand what's driving current audience interest and predict future trends."
    )

    recent_titles = df_filtered[df_filtered["year"] >= 2023].copy()

    if len(recent_titles) > 0:
        # Keep only titles with valid release timing and positive popularity
        recent_titles = recent_titles[
            (recent_titles["days_since_release"].notna())
            & (recent_titles["days_since_release"] > 0)
            & (recent_titles["popularity"].notna())
            & (recent_titles["popularity"] > 0)
        ].copy()

        # Prevent negative/NaN velocities and keep only meaningful growth
        recent_titles["velocity"] = (
            recent_titles["popularity"] / recent_titles["days_since_release"]
        )
        rising = recent_titles[recent_titles["velocity"] > 0].nlargest(10, "velocity")

        if len(rising) == 0 or rising["velocity"].max() <= 0:
            st.info("No recent titles with measurable positive velocity.")
        else:
            x_max = rising["velocity"].max() * 1.1
            ...
            fig.update_layout(
                xaxis=dict(
                    title="Velocity Score (Popularity/Day)",
                    color="#B3B3B3",
                    gridcolor="#262626",
                    range=[0, x_max],
                ),
...
def dashboard_6_head_to_head(df, filters):
    ...
    st.markdown(
        '<p class="subtitle">Compare genres or titles side-by-side for data-driven final decisions</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="dashboard-description"><strong>Business Problem:</strong> Should we choose Genre A or Genre B? Title strategy X or Y? This comparison engine provides direct side-by-side analysis across 7+ metrics with auto-calculated winner. Use this for final investment decisions when choosing between alternatives.</div>',
        unsafe_allow_html=True,
    )

    # Comparison Mode
    comparison_mode = st.radio(
        "Comparison Mode", ["Genre vs Genre", "Title vs Title"], horizontal=True
    )
    ...
    if comparison_mode == "Genre vs Genre":
        ...
        all_genres = sorted(
            df_filtered["primary_genre"].value_counts().head(15).index.tolist()
        )
        ...
        with col2:
            genre_b = st.selectbox(
                "Select Genre B", all_genres, index=min(1, len(all_genres) - 1)
            )
        ...
        metrics_radar = {
            "Popularity": [
                data_a["popularity"].mean() / df_filtered["popularity"].max() * 100
                if len(data_a) > 0
                else 0,
                data_b["popularity"].mean() / df_filtered["popularity"].max() * 100
                if len(data_b) > 0
                else 0,
            ],
            "Quality": [
            ...
            "Votes": [
                data_a["vote_count"].mean() / df_filtered["vote_count"].max() * 100
                if len(data_a) > 0
                else 0,
                data_b["vote_count"].mean() / df_filtered["vote_count"].max() * 100
                if len(data_b) > 0
                else 0,
            ],
        }
        ...
        st.markdown(
            "**Table Analysis:** Direct metric comparison. Red highlighting = winner for each row. Overall winner = most rows won. Use this for quick decision-making when time is critical."
        )
        ...
        comp_df["Winner"] = comp_df.apply(
            lambda row: genre_a
            if row[genre_a] > row[genre_b]
            else genre_b
            if row[genre_b] > row[genre_a]
            else "Tie",
            axis=1,
        )
        ...
        winner = (
            genre_a if score_a > score_b else genre_b if score_b > score_a else "TIE"
        )
        winner_score = max(score_a, score_b)

        if winner != "TIE":
            st.markdown(
                f'<div class="winner-badge">🏆 WINNER: {winner} ({winner_score}/{len(comp_df)} metrics)</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="winner-badge">🤝 TIE - Both equally matched</div>',
                unsafe_allow_html=True,
            )
        ...
        with col1:
            ...
            st.markdown(
                "**Chart Analysis:** Diverging bar chart for direct side-by-side comparison. Bars extend from center - LEFT (red) = Genre A, RIGHT (blue) = Genre B. Longer bars = better performance on that metric. Easy visual winner identification."
            )
            ...
            values_a = [-metrics_radar[k][0] for k in metrics_list]
            ...
            values_b = [metrics_radar[k][1] for k in metrics_list]
        ...
        with col2:
            ...
            st.markdown(
                "**Chart Analysis:** Historical engagement trends. Rising lines = growing genres. Falling = declining. Invest in upward trends."
            )
            trend_a = (
                data_a[data_a["year"] >= 2018]
                .groupby("year")["engagement_score"]
                .mean()
                .reset_index()
            )
            trend_b = (
                data_b[data_b["year"] >= 2018]
                .groupby("year")["engagement_score"]
                .mean()
                .reset_index()
            )
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    title="Avg Engagement", color="#B3B3B3", gridcolor="#262626"
                ),
                paper_bgcolor="#0A0A0A",
                plot_bgcolor="#0A0A0A",
                font=dict(color="#FFFFFF"),
            ...
            min_year = int(
                min(
                    trend_a["year"].min() if len(trend_a) > 0 else 9999,
                    trend_b["year"].min() if len(trend_b) > 0 else 9999,
                )
            )
            ...
            trend_direction_a = trend_direction(trend_a["engagement_score"])
            trend_direction_b = trend_direction(trend_b["engagement_score"])
            ...
            if len(trend_a) > 0 and len(trend_b) > 0:
                ...
                create_insight_panel(
                    ...
                    [
                        f"<span class='insight-highlight'>{winner}</span> shows stronger historical engagement with a {winner_trend} trajectory.",
                        f"{genre_a}: {trend_direction_a}; {genre_b}: {trend_direction_b} from 2018 to {int(max(trend_a['year'].max(), trend_b['year'].max()))}.",
                        f"{'Invest in the growing side' if 'growing' in [trend_direction_a, trend_direction_b] else 'Both are declining; pivot or differentiate'} based on momentum.",
                    ],
                )
            else:
                create_insight_panel(
        ...
        with col2:
            title_b = st.selectbox(
                "Title B", top_titles, index=min(1, len(top_titles) - 1)
            )
        ...
        score_b = 3 - score_a
        winner = (
            title_a if score_a > score_b else title_b if score_b > score_a else "TIE"
        )
        ...
        if winner != "TIE":
            ...
            st.markdown(
                f'<div class="winner-badge">🤝 TIE</div>', unsafe_allow_html=True
            )
...
def main():
    ...
    with st.sidebar:
        ...
        if page == "Content Universe":
            ...
            st.markdown(
                '<div style="background: #1A1A1A; padding: 8px 12px; border-radius: 6px; border-left: 3px solid #6B7280; margin: -8px 0 10px 0;"><div style="color: #9CA3AF; font-size: 0.8rem;">ℹ️ Showing all content</div></div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown("---")
        st.markdown(
            '<div style="color:#B3B3B3;font-size:0.75rem;line-height:1.4;">Powered by TMDB API | Machine Learning: LightGBM, Prophet Based Regression | Network Analysis: NetworkX</div>',
            unsafe_allow_html=True,
        )
